...
try { const m = await import("./azure-api.js"); submitDrawing = m.submitDrawing || submitDrawing; } catch {}
...
(() => { const chip = document.querySelector(".preview-caption"); if (chip) chip.remove(); })();

/* Offscreen buffer to prevent flicker */
const previewBuffer = (() => { const c=document.createElement("canvas"); c.width=previewCanvas?previewCanvas.width:0; c.height=previewCanvas?previewCanvas.height:0; return c; })();
...
function buildAppearances(manifest, charId){
  const out=[]; (manifest.slides||[]).forEach((sl,idx)=>{ const has=Array.isArray(sl.characters)&&sl.characters.some(c=>(c.id||"").toLowerCase()===charId); if(has) out.push(idx); });
  return out;
}
function perSlidePaintKey(story, charId, slide1){ return `perSlidePaint:${story}:${charId}:${slide1}`; }
...
function getSpriteBox() {
  ...
  return { width:size, height:size, x:Math.round((bgCanvas.width-size)/2), y:Math.round((bgCanvas.height-size)/2) };
}
function drawWhiteBG(){ bgCtx.fillStyle="#ffffff"; bgCtx.fillRect(0,0,bgCanvas.width,bgCanvas.height); }
function layoutAndRedraw(){
  const w=innerWidth, h=innerHeight;
  [bgCanvas,drawCanvas,spriteCanvas].forEach(c=>{ c.width=w; c.height=h; c.style.width=w+"px"; c.style.height=h+"px"; });
  drawWhiteBG();
  if (outlineLoaded){
    const box=getSpriteBox(); allowedArea={...box};
    sctx.clearRect(0,0,spriteCanvas.width,spriteCanvas.height);
    sctx.imageSmoothingEnabled=true; sctx.drawImage(outlineImg, box.x, box.y, box.width, box.height);
  }
  schedulePreview();
}
...
let brushSize=18, brushColor="#2ad0ff", opacity=1.0;
let prevX=null, prevY=null, zoomLevel=1;

/* ===== Painting toolbox wiring (new) ===== */
...
let activeBrush = "paintbrush"; // UI-only; painting engine unchanged

function setTool(tool){ currentTool = tool; } // keep the same global

function showBrushUI(){
  document.getElementById("sliders")?.classList.add("show");
  brushPopup?.classList.add("show");
  colorPopup?.classList.remove("show");
}
function showNonBrushUI(){
  document.getElementById("sliders")?.classList.add("show");
  brushPopup?.classList.remove("show");
  colorPopup?.classList.remove("show");
}
function activate(btn){
  [brushBtn, eraserBtn, smudgeBtn].forEach(b => b && b.classList.remove("active"));
  btn?.classList.add("active");
}

brushBtn?.addEventListener("click", () => { setTool("draw");  activate(brushBtn);  showBrushUI(); });
eraserBtn?.addEventListener("click",()=> { setTool("erase"); activate(eraserBtn); showNonBrushUI(); });
smudgeBtn?.addEventListener("click",()=> { setTool("draw");  activate(smudgeBtn); showNonBrushUI(); });

brushPopup?.querySelectorAll("button[data-brush]").forEach(b=>{
  b.addEventListener("click", ()=>{ activeBrush=b.dataset.brush; /* UI only */ });
});
...
document.addEventListener("click", (e) => {
  if (!e.target.closest("#toolbox")) { colorPopup?.classList.remove("show"); brushPopup?.classList.remove("show"); }
});
colorInput?.addEventListener("input", () => { brushColor = colorInput.value; if(colorSwatch) colorSwatch.style.background = brushColor; });
sizeSlider?.addEventListener("input", () => { brushSize = parseInt(sizeSlider.value,10); });
opacitySlider?.addEventListener("input", () => { opacity = parseFloat(opacitySlider.value)/100; });
...
function isInBounds(x,y){ return x>=allowedArea.x && x<=allowedArea.x+allowedArea.width && y>=allowedArea.y && y<=allowedArea.y+allowedArea.height; }
...
function schedulePreview(){ if(previewScheduled) return; previewScheduled=true; requestAnimationFrame(()=>{ previewScheduled=false; drawPreview(); }); }
...
function drawStroke(e){
  ...
  prevX=x; prevY=y; schedulePreview();
}
...
function applyZoom(){ [bgCanvas,drawCanvas,spriteCanvas].forEach(c=>{ c.style.transformOrigin="center center"; c.style.transform=`scale(${zoomLevel})`; }); schedulePreview(); }

/* Save dropdown (unchanged API stub so external calls still work) */
function toggleSaveOptions(){ /* no-op in new UI; kept for compatibility */ }
...
async function loadCSVMatrix(url){
  ...
  if(!W||!H) throw new Error(`Empty/invalid CSV: ${url}`); return { mat, W, H };
}
...
/* Mask discovery (your current single-frame loop preserved) */
async function findMaskSets(storyIdDash, charId){
  const storyFolder=resolveStoryFolder(storyIdDash);
  const base=`images/frames/${storyFolder}`; const out=[]; let misses=0;
  for(let n=1;n<2;n++){
...
async function sendToStoryboard() {
  ...
  try {
    ...
    baseCanvas.width = baseImg.width; baseCanvas.height = baseImg.height;
    ...
    if (selectedChar === "tortoise") {
      const merged=document.createElement("canvas");
      merged.width=cropCanvas.width; merged.height=cropCanvas.height;
      ...
      location.href = `storyboard.html?${q.toString()}`; return;
    } else {
      ...
      for (const { frame, prefix } of sets) {
        ...
        for (const regionId of uniqueIds) {
          ...
          const cx=maskCanvas.getContext("2d"); const imgData=cx.createImageData(W,H);
          for (let y=0;y<H;y++){ for (let x=0;x<W;x++){ if (mat[y][x]===regionId){ const idx=(y*W+x)*4; imgData.data[idx]=255; imgData.data[idx+1]=255; imgData.data[idx+2]=255; imgData.data[idx+3]=255; } } }
          cx.putImageData(imgData,0,0);

          const scaledMask=document.createElement("canvas"); scaledMask.width=width; scaledMask.height=height;
    ...
    const storyFolder=resolveStoryFolder(selectedStory || "tortoise-hare");
    for (const [frame, regions] of Object.entries(bySlide)) imageOnlyBySlide[frame]=regions.map(r=>r.img);
    ...
    if (firstImg) { try { await submitDrawing(sessionCode, selectedChar, firstImg, uid); } catch(e){ console.warn("[submitDrawing]", e); } }
...
/* ---------- Expose for buttons (APIs unchanged) ---------- */
...
function updateSliderFill(slider){ if(!slider) return; const value=((slider.value-slider.min)/(slider.max-slider.min))*100; slider.style.setProperty("--percent", `${value}%`); }
[sizeSlider, opacitySlider].forEach(sl=>{ if(!sl) return; updateSliderFill(sl); sl.addEventListener("input", ()=>updateSliderFill(sl)); });

/* === Appearances-only model + preview === */
let slidesManifest=null, appearances=[], appearCursor=0;
/* (…unchanged preview code…) */
function ensurePreviewDimsFor(bgIm){
  const sceneW=bgIm.naturalWidth||bgIm.width||1600, sceneH=bgIm.naturalHeight||bgIm.height||900;
  const MAX_W=320, MAX_H=200, scale=Math.min(MAX_W/sceneW, MAX_H/sceneH);
  const cw=Math.max(1,Math.round(sceneW*scale)), ch=Math.max(1,Math.round(sceneH*scale));
  if(previewCanvas&&(previewCanvas.width!==cw||previewCanvas.height!==ch)){ previewCanvas.width=cw; previewCanvas.height=ch; previewCanvas.style.width=`${cw}px`; previewCanvas.style.height=`${ch}px`; }
  if(previewBuffer.width!==cw||previewBuffer.height!==ch){ previewBuffer.width=cw; previewBuffer.height=ch; }
  return { sceneW, sceneH };
}

let previewToken=0;
async function drawPreview(){
  if(!pctx || !slidesManifest || !appearances.length){ if(pctx) pctx.clearRect(0,0,previewCanvas.width,previewCanvas.height); return; }
  const myToken=++previewToken, globalIdx=appearances[appearCursor]; const slide=slidesManifest.slides[globalIdx]; if(!slide) return;
  try{
    ...
    const vw=sceneW*scale, vh=sceneH*scale, ox=(previewBuffer.width-vw)/2, oy=(previewBuffer.height-vh)/2;
    pb.clearRect(0,0,previewBuffer.width,previewBuffer.height); pb.drawImage(bgIm,ox,oy,vw,vh);

    const charCfg=(slide.characters||[]).find(c => (c.id||"").toLowerCase()===selectedChar);
    if(charCfg){
      const dx=ox+(charCfg.x/100)*vw, dy=oy+(charCfg.y/100)*vh, dw=(charCfg.w/100)*vw, dh=dw;
      const sprite=document.createElement("canvas"); sprite.width=Math.max(1,Math.round(dw)); sprite.height=Math.max(1,Math.round(dh));
      ...
      try{
        ...
        if(await urlExists(csvURL)){ const { mat,W,H }=await loadCSVMatrix(csvURL); const mask=await matrixToMaskCanvas(mat,W,H,sprite.width,sprite.height); scx.globalCompositeOperation="destination-in"; scx.drawImage(mask,0,0); scx.globalCompositeOperation="source-over"; }
      }catch{}
      pb.drawImage(sprite,dx,dy,dw,dh);
      try{ const storyFolder=resolveStoryFolder(selectedStory || "tortoise-hare"); const frame1=`images/frames/${storyFolder}/frame${globalIdx+1}/${selectedChar}/${selectedChar}1.png`; if(await urlExists(frame1)){ const ol=await loadImageCached(frame1); pb.drawImage(ol,dx,dy,dw,dh); } }catch{}
    }
    if(myToken===previewToken){ pctx.clearRect(0,0,previewCanvas.width,previewCanvas.height); pctx.drawImage(previewBuffer,0,0); }
  }catch{ pctx.clearRect(0,0,previewCanvas.width,previewCanvas.height); }
}

async function gotoAppearance(n){
  if(!appearances.length) return; if(n<0||n>=appearances.length) return;
  appearCursor=n; const outlineURL=await resolveOutlineURLForSlide(appearances[appearCursor]+1); outlineImg.src=outlineURL;
  restoreCurrentAppearance(); schedulePreview();
}
...
function restoreCurrentAppearance(){
  ...
  if(!dataURL) return;
  const img=new Image(); img.onload=()=>{ ctx.drawImage(img,0,0,img.width,img.height, allowedArea.x,allowedArea.y,allowedArea.width,allowedArea.height); schedulePreview(); }; img.src=dataURL;
}
...
(async function boot(){
  ...
  const m = manifest || { slides: [] };
  const a = buildAppearances(m, selectedChar);
  Object.assign(window, { _slides:m, _apps:a }); // debug aid

  // init sliders/colors to current values
  if (sizeSlider)    sizeSlider.value    = String(brushSize);
  if (opacitySlider) opacitySlider.value = String(Math.round(opacity*100));
  if (colorInput)    colorInput.value    = brushColor;
  if (colorSwatch)   colorSwatch.style.background = brushColor;

  // show brush popup by default (brush is active)
  document.getElementById("sliders")?.classList.add("show");
  document.getElementById("brushPopup")?.classList.add("show");

  if (a.length){ slidesManifest=m; appearances=a; await gotoAppearance(0); }
  else { slidesManifest=m; appearances=a; schedulePreview(); }
})();
...
async function sendToStoryboard() {
  ...
  try {
    ...
    if (selectedChar === "tortoise") {
      ...
      // 1️ Draw the paint layer first
      mctx.drawImage(cropCanvas, 0, 0);
  
      // 2️ Draw the outline on top so black lines are preserved
      const baseImg = await loadImage(baseFrameURL); // outline image
      mctx.drawImage(baseImg, 0, 0, merged.width, merged.height);
  
      // 3️ Export the merged result
      const dataUrl = merged.toDataURL("image/png");
